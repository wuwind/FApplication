    ...
    private void showIm(final FrameLayout layout, TipData... tipDatas) {
        ...
        for (TipData data : tipDatas) {
            ...
            for (View view : views) {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
                    location[1] -= DisplayUtil.getStatusBarHeight(activity);
                }
            ...
            Point displayMetrics = DisplayUtil.getDisplayMetrics(activity);
            ...
            if (x < 0) {
                x = 0;
            } else if (x + imageW > displayMetrics.x) {
                x = displayMetrics.x - imageW;
            }
            if (y < 0) {
                y = 0;
            } else if (y + imageH > displayMetrics.y) {
                y = displayMetrics.y - imageH;
            }
